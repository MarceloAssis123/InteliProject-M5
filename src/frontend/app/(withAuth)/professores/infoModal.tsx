...
import { editError, editSuccess } from '@/utils/toastMessages';
...
// Este componente "InfoModal" é responsável por renderizar o modal de informações de um professor
export default function InfoModal({ data, setProfessors, setIsModalVisible, isModalVisible, selectedProfessorId, getWorkshops, messageApi }: { data: any, setProfessors: any, setIsModalVisible: any, isModalVisible: any, selectedProfessorId: number | undefined, getWorkshops: any, messageApi: any }) {
    // Armazenamento do estado de edição
    ...
    const saveInfo = (professor: any) => {
        async function editInfo() {
            ...
            if (response.ok) {
                ...
                setEditMode(false);
                console.log(professor, professorData.id)
                setProfessorData(professor);
            } else {
                editError(messageApi);
            }
        }
    ...
    // Função que atualiza os dados do professor
    useEffect(() => {
        if (data) {
    ...
    return (
        <Modal centered style={{ top: '0' }} width={600} title={editMode ? "Editar Professor" : addClassroomMode ? "Turmas do professor" : "Detalhes do Professor"} open={isModalVisible} onOk={handleOk} onCancel={() => { setEditMode(false); handleCancel(); }} footer={null} >
            {!editMode ?
                addClassroomMode ?
                    <>
                        <AddClassroomModeContent messageApi={messageApi} getWorkshops={getWorkshops} data={data} selectedProfessorId={selectedProfessorId} setAddClassroomMode={setAddClassroomMode} />
                    </>
                    :
                    <>
