...
import { createWorkshopError, createWorkshopSuccess } from "@/utils/toastMessages";
...
export default function FormWorkshopRegister({ entity, url, messageApi }: { entity: string, url: string, messageApi: any }) {
	...
	return (
		<CustomFormRegister name={`register${entity}`} onFinish={async (values: any) => {
			try {
				...
				if (!response.ok) {
					createWorkshopError(messageApi);
					throw new Error(`Erro ao cadastrar ${entity}`);
				}

				createWorkshopSuccess(messageApi);
				const responseData = await response.json();
				console.log(responseData);
			} catch (error) {
				console.error(error);
