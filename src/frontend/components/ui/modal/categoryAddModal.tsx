...
import { createCategoryError, createCategorySuccess } from '@/utils/toastMessages';

// Este é um modal que permite a adição de uma nova categoria
export default function CategoryAddModal({ visible, onCreate, onCancel, messageApi }: { visible: boolean, onCreate: any, onCancel: any, messageApi: any }) {
    ...
    form.validateFields()
      .then(async (values) => {
        ...
        try {
          ...
          if (!response.ok) {
            createCategoryError(messageApi);
            throw new Error('Falha ao cadastrar categoria');
          }

          createCategorySuccess(messageApi);
